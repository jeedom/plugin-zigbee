...
$group_data = zigbee::request($eqLogic->getConfiguration('instance',1),'/group/info',array('id'=>explode('|',$eqLogic->getLogicalId())[1]));
...
$('.bt_addToZigbeeGroup').off('click').on('click',function(){
  ...
  for(var i in listAddGroup){
    inputOptions.push({value : listAddGroup[i].ieee,text : listAddGroup[i].name});
  }
  if(inputOptions.length == 0){
    $('#div_groupeZigbeeAlert').showAlert({message: '{{Aucun module ne peut etre ajouté dans ce groupe}} ', level: 'warning'});
    return;
  }
  bootbox.prompt({
    ...
    callback: function (device_result) {
      ...
      jeedom.zigbee.group.add_device({
        ...
        error: function (error) {
          $('#div_groupeZigbeeAlert').showAlert({message: error.message, level: 'danger'});
        },
        success: function () {
          $('#div_groupeZigbeeAlert').showAlert({message: '{{Module ajouté avec succès au groupe}} ', level: 'success'});
