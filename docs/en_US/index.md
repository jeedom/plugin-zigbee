...
Updates are carried out automatically in the event of availability or if the module requests it. It is possible to force the update of a module from the tab **Shares** from the module configuration window on the device page.
...
**Touch link** *(or Lightlink)* is a particular function of Zigbee which allows the controller to send management orders to a module on condition of being very close to it *(less than 50 centimeters)*. This is useful, for example, to reset bulbs that do not have a physical button.
...
>**I have errors ````can not send to device```` Where ````send error```` Where ````Message send failure````**
>
...
> -	the module is in ZHA, so it's a concern of inclusion. In the tab **Stock** of the module configuration, there is a button **Reset module** allowing to force post-inclusion actions. Care must be taken to keep the module awake if it is on battery.
